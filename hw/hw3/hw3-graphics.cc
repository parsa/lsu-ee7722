...
/// DO NOT MODIFY THIS FILE.
 /// It will not be copied by the TA bot.
...
typedef enum { P_CPU, P_GPU_Base, P_GPU_Sol, P_PHYSICS_SIZE } P_Physics;
...
public:
  ...
  int duration_physics_id;
  double duration_physics_sum;
  double duration_physics_deadline;
  double duration_physics;
  int duration_physics_samples;

  cudaDeviceProp cuda_prop;  // Properties of cuda device (GPU, cuda version).
  GPU_Info gpu_info;
  int opt_grid_size, opt_block_size;
...
World::init_graphics()
{
  ...
  eye_location = pCoor(27.8,14.0,-46.3);
  eye_direction = pVect(-0.42,-0.09,0.9);

  c.platform_xmin = -40; c.platform_xmax = 40;
  c.platform_zmin = -40; c.platform_zmax = 40;
  texid_syl = pBuild_Texture_File("gpup.png",false,255);
  texid_emacs = pBuild_Texture_File("mult.png", false,-1);

  opt_light_intensity = 100.2;
  light_location = pCoor(c.platform_xmax,c.platform_xmax,c.platform_zmin);
...
World::platform_update()
{
  ...
  const float delta_x = ( c.platform_xmax-c.platform_xmin ) / tile_count * ep;
  const float zdelta = ( c.platform_zmax-c.platform_zmin ) / tile_count * ep;
  ...
  for ( int i = 0; i < tile_count; i++ )
    {
      const float x0 = c.platform_xmin + i * delta_x;
      const float x1 = x0 + delta_x;
      const float y = 0;
      for ( float z = c.platform_zmin; z < c.platform_zmax; z += zdelta )
        {
      ...
      for ( int i=0; i<c.chain_length; i++ )
        {
          ...
          else if ( i == 0 && c.opt_head_lock
                    || i == c.chain_length-1 && c.opt_tail_lock )
...
# define BLINK(txt,pad) ( blink_visible ? txt : pad )
  ...
  ogl_helper.fbprintf
    ("Physics On: %-11s (press 'a')  "
     " *** Time Per Time Step: %7.3f µs ***   %s  opt_testX %d%d ('x' or 'X')\n",
     opt_physics == P_CPU ? BLINK("   CPU",optimized?"  CPU":"unoptimized") :
     opt_physics == P_GPU_Base ? BLINK("GPU Base"," GPU Base") :
     opt_physics == P_GPU_Sol ? "GPU Sol" : "???",
     1e6*duration_physics,
     opt_pause ? BLINK("PAUSED, 'p' to unpause, SPC or S-SPC to step.","") :
     "Press 'p' to pause.",
     c.opt_test1, c.opt_test2
     );
  ...
  //
  glStencilOp(GL_KEEP,GL_KEEP,GL_KEEP);
  glStencilFunc(GL_EQUAL,viewer_shadow_volume,0xffff); // ref, mask
...
World::cuda_init()
{
  ...
  // Run an nvcc-compiled routine that collects information on kernels
  // and device addresses of __constant__ symbols.
  //
  cuda_setup(&gpu_info);

  // Print information about the kernels that was collected by cuda_setup.
  //
...
World::cb_keyboard()
{
  ...
  case 'a': case 'A': opt_physics++;
    if ( opt_physics == P_PHYSICS_SIZE ) opt_physics = 0;
    break;
  ...
  case 'h': case 'H': c.opt_head_lock = !c.opt_head_lock; break;
  case 't': case 'T': c.opt_tail_lock = !c.opt_tail_lock; break;
  case 'l': case 'L': opt_move_item = MI_Light; break;
  case 'n': case 'N': opt_platform_texture = !opt_platform_texture; break;
  case 'p': case 'P': opt_pause = !opt_pause; break;
  case 's': case 'S': balls_stop(); break;
  case 'x': c.opt_test1 = !c.opt_test1; break;
  case 'X': c.opt_test2 = !c.opt_test2; break;
  case ' ':
  ...
  c.gravity_accel.y = opt_gravity ? -opt_gravity_accel : 0;
