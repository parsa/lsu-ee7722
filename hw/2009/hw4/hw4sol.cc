...
/// Problem 3
 ...
 // The index array refers to the same vertex multiple times, and so
 // OpenGL could just send that vertex to the GPU once, and possibly just
 // vertex-shade it once. But does it? The data below are from runs in
 // which Pattern Levels was set to 500 (so there would be a significant
 // amount of work to do). The runs in which a buffer object is used for
 // the data are significantly faster. This suggests that OpenGL is
 // sending vertices over multiple times rather than keeping track of
 // which vertices have already been sent.

 // The transfer rates shown above are based on the assumption that
 // OpenGL does re-send vertices, so the GPU and CPU times match
 // expectations.
...
private:
  ...
  GLuint gpu_index_buffer;    /// PROBLEM 1 SOLUTION
  GLuint gpu_vertex_buffer;   /// PROBLEM 2 SOLUTION
  bool gpu_data_stale;        /// PROBLEM 2 SOLUTION
...
Tube::init()
{
  ...
  glGenBuffers(1,&gpu_index_buffer);  /// PROBLEM 1 SOLUTION
  glGenBuffers(1,&gpu_vertex_buffer); /// PROBLEM 2 SOLUTION
  gpu_data_stale = true;              /// PROBLEM 2 SOLUTION
...
Tube::render()
{
  ...
  /// PROBLEM 2 SOLUTION
  //
  if ( opt_v_buffering == 2 && gpu_data_stale )
    {
      gpu_data_stale = false;
      glBindBuffer(GL_ARRAY_BUFFER,gpu_vertex_buffer);
      const int data_chars = num_vtx * sizeof(vertex_info[0]);
      glBufferData
        (GL_ARRAY_BUFFER, data_chars, vertex_info, GL_DYNAMIC_COPY);
      glBindBuffer(GL_ARRAY_BUFFER, 0);
      pError_Check();
      num_bytes += data_chars;
    }
  ...
  if ( !index_array )
    {
      ...
      const int data_chars = num_indices * sizeof(index_array[0]);
      glBufferData
        (GL_ELEMENT_ARRAY_BUFFER, data_chars, index_array, GL_STATIC_DRAW);
  ...
  case 2:
    ...
    //
    glBindBuffer(GL_ARRAY_BUFFER, gpu_vertex_buffer);
    glVertexPointer(3,GL_FLOAT,sizeof(Vertex_Info),NULL);
    glNormalPointer(GL_FLOAT,sizeof(Vertex_Info),(void*)sizeof(pCoor));
    glEnableClientState(GL_NORMAL_ARRAY);
    glEnableClientState(GL_VERTEX_ARRAY);
    pError_Check();
