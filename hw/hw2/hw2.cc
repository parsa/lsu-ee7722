  ...
  if ( coords_stale )
    {
      ...
      for ( int i = 1; i < segments_per_helix; i++ )
        {
          const double eta = i * delta_eta;

          // The previous point on center of wire.
          pCoor p0 = p;
          pCoor c0(0,p0.y,0);  // Point on axis of helix closest to p0.

          // Compute current point on center of wire.
          p.y += delta_y;  p.x = cos(eta);  p.z = sin(eta);

          pCoor p1 = p;
          pCoor c1(0,p1.y,0);

          // Vectors pointing to helix axis. Used to compute points
          // on wire.
          pVect n0 = small_r * pVect(p0,c0);
          pVect n1 = small_r * pVect(p1,c1);

          // Find points on surface of wire.
          for ( int j = 0; j < seg_per_small_circle; j++ )
            {
              // Get index (element number) of next item to be placed
              // in prep_coords.
              //
      ...
      // Move from working storage to more permanent storage.
      //
