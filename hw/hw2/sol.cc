...
public:
           ...
           int R_requested = 1 )
  {
    ...
    const int version_s_shared_bytes = 3 * db.x * sizeof(float);

    // Compute the largest possible value of R taking into account
    // the static shared memory used by the routine and the total
    ...
    const int shared_req = func_attr->attr.sharedSizeBytes + shared_bytes;
    if ( shared_req >= int(cuda_prop.sharedMemPerBlock) )
      {
        printf("Requesting too much shared memory, %d (%d available)\n",
               shared_req, int(cuda_prop.sharedMemPerBlock) );
        return;
      }
