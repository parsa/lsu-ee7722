...
public:
  ...
  // Members for Fall 2017 Homework 1;

  Ball *marker_red, *marker_blue, *marker_khaki;

  void render_p0();
  void render_p1();
  void render_p2();
  Ball *thing_1_apex, *thing_1_base;
  Balls thing_1_ring;
  pVectorI<Ball> chain_starts;
  Balls thing_1_pseudo;
  void objects_erase();
  float opt_height;
  int opt_version;
  int opt_layers;
...
World::init_graphics()
{
  ...
  eye_location = pCoor(39.6,12.8,-21.8);
  eye_direction = pVect(-0.93,-0.27,.25);
...
{
  const float shininess_ball = 2;
  //  pColor spec_color(0.2,0.2,0.2);
  pColor spec_color(1,0,0);
...
# define BLINK(txt,pad) ( blink_visible ? txt : pad )
  ...
  ogl_helper.fbprintf
    ("Version: %s ('v' to change)  "
     "Tryout 1: %s  ('y' to change)  Tryout 2: %s  ('Y' to change)\n",
     opt_version == 0 ? "p0 -- EXPERIMENT" :
     opt_version == 1 ? "p1 -- PROBLEM 1" :
     opt_version == 2 ? "p2 -- PROBLEM 2" : "????",
  ...
  glBlendFunc(GL_ONE,GL_CONSTANT_ALPHA);  // src, dst
  ...
  glColor3fv(0.8 * color_lsu_spirit_purple);
  glDrawArrays(GL_QUADS,half_elements+4,half_elements-4);
  glDisable(GL_BLEND);
...
World::cb_keyboard()
{
  ...
  case 'v': case 'V':
    opt_version++; if ( opt_version > 2 ) opt_version = 0;
    break;
