...
//
 ...
 //
 ///  Most of the solution should be in file hw04-shdr.cc, but some modifications
...
public:
  ...
  GLuint rotations_bo, pos_rad_bo, colors_bo;

};

enum { OH_Links = 1, OH_Platform = 2, OH_Sphere = 4 };

enum { OSh_Sphere = 0, OSh_Problem_0, OSh_Problem_1 };
      ...
      if ( !hide_sphere )
        {
          ...
          switch ( opt_shader ) {
          case OSh_Sphere: sphere.render_bunch_render(); break;
          case OSh_Problem_0: case OSh_Problem_1: render_p1(); break;
          default: assert( false );
          }
        }
  ...
  for ( int i=1; i<8; i++ )
    {
      ...
      // Rotate ball so it looks good for 2017 Homework 4.
      ball->orientation_set(dir_up,M_PI);
...
World::render_p1()
{
  ...
  if ( sphere_coords.empty() )
    ...
    // affecting its contents change.
    {
      ...
      const double delta_t = 2.0 / opt_slices;
      ...
      for ( int i=1; i<opt_slices; i++ )
        {
          ...
          const double srt = t < 1 ? pow(t,0.5) : 2.0-pow(2-t,0.5);
          ...
          pCoor pt0(r * cos(theta0), y, r * sin(theta0), theta0);
          pCoor pt1(r * cos(theta1), y, r * sin(theta1), theta1);
  ...
  vector<pMatrix> rotations;
  vector<pCoor> pos_rad;
  ...
  for ( Ball *ball: balls )
    {
      ...
      center.w = ball->radius;
      pos_rad.push_back(center);
      rotations.push_back(pMatrix_Rotation(ball->orientation));
    }

  if ( rotations_bo == 0 )
    for ( auto boptr: { &rotations_bo, &colors_bo, &pos_rad_bo } )
      glGenBuffers(1,boptr);

  switch ( opt_shader ) {
  case OSh_Problem_0: sp_prob0->use(); break;
  case OSh_Problem_1: sp_prob1->use(); break;
  default: assert( false ); // This can't happen.
  };

#define TO_BUFFER_OBJECT(vec,binding)                                         \
  glBindBuffer(GL_ARRAY_BUFFER,vec##_bo);                                     \
  glBufferData(GL_ARRAY_BUFFER,vec.size()*sizeof(vec[0]),vec.data(),          \
               GL_STREAM_DRAW);                                               \
  glBindBufferBase(GL_SHADER_STORAGE_BUFFER,binding,vec##_bo);

  TO_BUFFER_OBJECT(rotations,1);
  TO_BUFFER_OBJECT(pos_rad,2);
  TO_BUFFER_OBJECT(colors,3);
# undef TO_BUFFER_OBJECT

  // Initialize a uniform used by the lighting routine.
  glUniform1i(2, light_state_get());

  glUniform3i(3, opt_tryout1, opt_tryout2, opt_normal_sphere);
  glUniform1i(4, opt_spirals);
  ...
  glDrawArraysInstanced
    (GL_TRIANGLE_STRIP,      // Type of primitive.
     0,                      // Index of starting vertex.
     sphere_coords.size(),   // Number of vertices per instance.
     pos_rad.size()          // Number of instances.
     );
