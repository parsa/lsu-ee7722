...
World::render_p1()
{
  ...
  /// Problem 1 Note: 
  //    The code below should be modified, rearranged, etc.
  ...
  /// Problem 1 Note:
  //
  // The code above is based on demo-7-vtx-arrays.
  // The code below is specific to this problem.
  // For this problem modify both the code above and below.
  ...
  for ( Ball *ball: balls )
    {
      ...
      float __attribute__((unused)) radius = ball->radius;
...
World::render_p2()
{
  ...
  if ( sphere_coords.empty() )
    ...
    // affecting its contents change.
    {
      ...
      for ( int i=0; i<=opt_slices; i++ )
        {
          const double t = i * delta_t;
          const double srt = pow(t,0.5);

          // Placeholder: Just make one triangle and exit the loop early.
          sphere_coords.push_back
            ( pCoor( 1, 0, 0 ) );
          sphere_coords.push_back
            ( pCoor( cos(delta_theta), 0, sin(delta_theta) ) );
          break;
        }
  ...
  /// Problem 2 Note: 
  //    The code above and below should be modified, rearranged, etc.
