...
//
 /// Final Exam Code, Based on Homework 4
 //
...
struct HW04_Stuff {
  // This is a member of world, named hw04.
...
World::init()
{
  ...
  opt_hw02_size = 0.1;
  hw04.bump_stale = true;
  variable_control.insert
    (opt_hw02_size,"Bump Size").set_on_change(hw04.bump_stale);
...
{
  /// 2019 Final Exam: Simplified code for Problem 1.
  ...
  if ( roption == RO_Shadow_Volumes ) return;
  if ( !opt_shader ) return;
      ...
      for ( float theta = 0;  theta < theta_stop;  theta += delta_theta )
        {
          ...
          e.cbot = e.ctop + vd + vd;
          hw04.bump_info.push_back(e);
        }
      hw04.bump_stale = false;
    }

  if ( opt_shader == 1 ) hw04.sp_strip_plus->use();
  else                   hw04.sp_points->use();
  ...
  glUniform4fv(5,1, 0.5*color_tan); // color_diamond
  ...
  if ( opt_shader == 2 )
    {
      // Send items needed by points shaders to construct the protrusions.
  ...
  //
  //  The code fragment used in the exam is a compacted version of the
  //  code below. The code below is itself a simplified version of the
  //  render_cylinder routine in which code for selecting the
  //  different shader versions has been removed.
  ...
  for ( auto& e: hw04.bump_info )
    {
      pCoor vtop(e.ctop), vl(e.cl);
      vtop.w = vl.w = 0;
  ...
  for ( auto& e: hw04.bump_info )
    {
      ...
      // render the sides using a loop.
      //
      pCoor pts[2][4] =
        {
          // Original Diamond
          { e.ctop, e.cr, e.cbot, e.cl },
      ...
      //
      // Each iteration emits vertices for an edge of the block. The
      // first edge is emitted twice: on the first and last
      // iteration. It is emitted a second time to complete the final
      // triangle strip.
      //
      for ( int i=0; i<5; i++ )
        {
...
World::render_cylinder(Render_Option roption)
{
  ...
  //
  //  Turn on textures and attach the syllabus texture to the first
  //  texture unit.
  //
  glEnable(GL_TEXTURE_2D);
  glBindTexture(GL_TEXTURE_2D,texid_syl);
  //
  //  No other significant changes have been made to the code below
  //  for the final exam. This means that textures will only work for
  //  the points shader, opt_shader == 2.
  ...
  if ( opt_shader )
    {
      ...
      glUniform3i(3, opt_tryout1, opt_tryout2, opt_tryout3 );
      glUniform1f(4, opt_tryoutf);
      glUniform4fv(5,1, color_tan * 0.5); // color_diamond
      glUniform4fv(6,1, color_olive_drab); // color_edge
    }

  if ( opt_shader == 2 )
    {
      // Send items needed to construct the protrusions.
  ...
  for ( auto& e: hw04.bump_info )
    {
      ...
      // Each iteration emits vertices for an edge of the block. The
      // first edge is emitted twice: on the first and last
      // iterations. It is emitted a second time to complete the final
      // triangle strip.
      //
      for ( int i=0; i<5; i++ )
        {
          ...
          // Prepare an array of coordinates in the correct order for a
          // triangle strip.
          //
          vector<pCoor> spts;
          ...
          if ( opt_shader )
          ...
          else
            {
              ...
              // the middle of a rendering pass.
