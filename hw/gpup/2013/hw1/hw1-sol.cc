...
World::ball_setup_2()
{
  ...
  /// SOLUTION
  pCoor center(12.5,12,-13.7);
  const float ball_radius = 0.5;
  const float radius = 0.5 * center.y;
  pVect up(0,radius,0);
  pVect right(radius,0,0);
  const double delta_theta = 2 * M_PI / chain_length;
  for ( int i=0; i<chain_length; i++ )
    {
      Ball* const ball = &balls[i];
      const double theta = i * delta_theta;
      ball->position = center + cos(theta) * up + sin(theta) * right;
      ball->velocity = pVect(0,0,0);
