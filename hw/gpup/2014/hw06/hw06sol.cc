...
//
 ...
 //
 /// Solution in this file and in hw06-shdr-lines.cc.
...
public:
  ...
  GLuint balls_bo;
  GLuint indices_bo;
  int indices_elts; // Number of elements (vertices) in indices bo.
  pShader *sp_fixed;
  pShader *sp_geo_triangles;
  pShader *sp_geo_lines;

  void cuda_init();
};


#include "hw06-graphics.cc"
...
World::init(int argc, char **argv)
{
  ...
  const char* const shader_code_triangles_file = "hw06-shdr-triangles.cc";

  const char* const shader_code_lines_file = "hw06-shdr-lines-sol.cc";

  sp_geo_triangles = 
    new pShader
    (shader_code_triangles_file,
     "vs_main_triangles();", "gs_main_triangles();", "fs_main();");

  sp_geo_lines = 
    new pShader
    (shader_code_lines_file,
     "vs_main_lines();", "gs_main_lines();", "fs_main();");

  sp_fixed = new pShader();

  // Note: CUDA isn't being used for anything other than displaying GPU info.
  cuda_init();
}
...
World::render_spiral2()
{
  ...
  if ( indices_elts != num_elts )
    {
      indices_elts = num_elts;
...
World::render_spiral3()
{
  ...
  //
  //  
  const int num_elts = ( chain_length - 1 ) * opt_segments;
  ...
  if ( indices_elts != num_elts )
    {
      /// SOLUTION
      //
      //  Code no longer uses the third component, inner/outer.
      //  Reduce the size of the indices array accordingly.
      //
      indices_elts = num_elts;
