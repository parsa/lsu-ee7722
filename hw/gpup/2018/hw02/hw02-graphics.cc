...
public:
  ...
  bool opt_shadows, opt_shadow_volumes_visible;
  ...
  void hw02_render(bool shadows);
  int opt_n_segs;

  enum FILL_METHOD { Fill_None, Fill_Wye, Fill_Louvers, FILL_SIZE };
  int opt_fill;
  const char* opt_fill_str[4] = {"NONE", "WYE", "LOUVERS", "??"};
...
World::render_objects(Render_Option option)
{
  ...
  if ( option == RO_Shadow_Volumes && opt_shadow_volumes_visible
       && opt_fill == Fill_Louvers )
    {
      hw02_render(true);
      return;
    }
...
# define BLINK(txt,pad) ( blink_visible ? txt : pad )
  ...
  ogl_helper.fbprintf
    ("Fill Mode: %s ('f')  Shadow Effect: %s ('w') Volume Visibility: %s ('W') "
     "Time Step: %8d  World Time: %11.6f  %s\n",
     opt_fill_str[opt_fill],
     opt_shadows ? "ON" : "OFF",
     opt_shadow_volumes_visible ? BLINK("ON ","   ") : "OFF",
...
World::cb_keyboard()
{
  ...
  case 'f': case 'F':
    opt_fill++; if ( opt_fill == FILL_SIZE ) opt_fill = 0; break;
  ...
  case 'W': opt_shadow_volumes_visible = !opt_shadow_volumes_visible; break;
