...
//
 /// Midterm Exam Code
 //
 // Adapted from Homework 2 and Homework 5.
 //
 /// Location of Code used in Midterm Exam
 //
 //    Problem 1:  
 //        This file, World::render_it
 //        At startup, shape shown in upper left and attached
 //         to the umbrella thing in scene 2 (the initial scene).
 //
 //    Problem 2:  
 //        File mt-shdr-links.cc, code in and around gs_main_1 and gs_main_2.
 //        Press 'v' to cycle between the different shaders. Best
 //          done in scene 2 (the initial scene).
 //
 //    Problem 3:
 //        This file, World::time_step_cpu
 //        Best demoed using scene 1. (Press '1').
 //        Turn on and off by pressing 'y' (tryout 1).
...
public:
  ...
  // Midterm Exam 2016
  void render_it(pCoor p1, pCoor p2, pCoor p3);
  Balls ref_balls;
};
...
World::init_graphics()
{
  ...
  texid_hw = pBuild_Texture_File("mt-assign.png", false,255);
...
World::render_objects(Render_Option option)
{
  ...
  if ( option == RO_Shadow_Volumes )
  ...
  else
    {
      ...
      if ( opt_shader == SO_Phong )     sp_phong->use();
      else if ( opt_shader == SO_Set_1 ) sp_set_1->use();
      else                              sp_set_2->use();
      ...
      render_it
        (sample_start, sample_start+pVect(0,1,0),
         sample_start+sample_size*dir_right);

      if ( ref_balls.size() )
        render_it
          ( ref_balls[0]->position, ref_balls[1]->position,
            ref_balls[2]->position);
...
World::init(int argc, char **argv)
{
  ...
  sp_phong = new pShader
    ("mt-shdr.cc",// File holding shader program.
  ...
  const char* const links_shader_code_path = "mt-shdr-links.cc";
...
World::ball_setup_1()
{
  ...
  pVect ay(0,1,0);
  pVect ptoeye(first_pos,eye_location);
  pNorm ax = cross(ay,ptoeye);

  const float bradius = 0.3;
  float rinner = 3 * bradius;
  float theta = 0;
  for ( int i=0; i<50; i++ )
    {
      Ball* const ball = balls += new Ball;
      const float rcurr = rinner + theta / ( 2 * M_PI ) * 2 * bradius;
      pVect c_to_p = rcurr * ( ax * cos(theta) + ay * sin(theta) );
      ball->position = first_pos + c_to_p;
      ball->locked = false;
      ball->velocity = pVect(0,0,0);
      ball->radius = 0.3;
      ball->contact = false;

      theta += 2 * bradius / rcurr;
...
World::ball_setup_2()
{
  ...
  assert( ref_balls == 3 );
  swap( ref_balls[1], ref_balls[2] );
  swap( ref_balls[0], ref_balls[1] );
...
void
World::render_it(pCoor p1, pCoor p2, pCoor p3)
{
  pCoor pa(2,4,0);
  pCoor pb(7,4,0);
  pCoor pc(1,3,0);
  ...
  /// SOLUTION -- Problem 1b.
#if 0
  // Note: If solution 1c omitted include the line below.
  pVect vg1 = p1 - pg;
  pMatrix m =   pMatrix_Translate move(vg1);
#endif

  /// SOLUTION -- Problem 1c.

  // Step 1: Transform to a local space.
  //
  // Translation moves pg to the origin.
  // The scale the object so that:
  //    vertices pg and pc are 1 unit apart along the y axis and
  //    vertices pc and pb are 1 unit apart along the x axis.
  // (Since points have the same z value, 0, there's no point in scaling z.)
  //
  pMatrix_Translate to_origin(-pg);
  pMatrix_Scale scale( 1/(pb.x-pc.x), 1/(pc.y-pg.y), 1);


  // Step 2: Rotate, scale, and translate for p1, p2, p3.
  //
  pVect ay(p1,p2);
  pVect v23(p2,p3);
...
World::time_step_cpu(double delta_t)
{
  ...
  float sep_spring_constant = 100 * opt_spring_constant;

  /// MIDTERM Problem 3 Code
  //
  const int chain_length = balls.size();
  if ( opt_tryout1 )
