...
public:
  ...
  // Members for Fall 2016 Homework 2
  void render_it_demo(pCoor a, pCoor b, pCoor c, int version);
  void render_it(pCoor a, pCoor b, pCoor c, int version);
  pCoor sample_a[6], sample_b[6], sample_c[6];
...
World::render_objects(Render_Option option)
{
  ...
  if ( option == RO_Shadow_Volumes )
  ...
  else
    {
      ...
      for ( int i=0; i<3; i++ )
        render_it_demo(sample_a[i],sample_b[i],sample_c[i],i);
      for ( int i=3; i<6; i++ )
        render_it(sample_a[i],sample_b[i],sample_c[i],i-3);
      if ( chain_starts.size() == 4 )
        {
          pCoor a = chain_starts[0]->position;
          pCoor b = chain_starts[1]->position;
          pCoor x = chain_starts[2]->position;
          pVect ax(a,x);
          pNorm dir_z = cross(x,b,a);
          pNorm dir_ab(a,b);
