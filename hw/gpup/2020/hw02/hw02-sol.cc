...
# define BLINK(txt,pad) ( blink_visible ? txt : pad )
  ...
  /// SOLUTION
  glBegin( !opt_tryout1 && opt_show || opt_show_lines
           ? GL_LINE_STRIP : GL_TRIANGLES);
  const pColor color_gray(0x555555);
  pVect v0(0,0,0), v2(0,0,0);
  ...
  for ( int slice = 0; slice < slices - 1; slice++ )
    {
      ...
      /// SOLUTION
      if ( !opt_tryout1 )
        slice_color = opt_show ? ( show_eta ? lsu_spirit_gold : color_gray ) :
          lsu_spirit_gold;

      /// SOLUTION
      if ( !opt_tryout1 && show_eta )
        {
          v0 = pVect( slice_r0, y0, 0 );
      ...
      for ( int seg = 0; seg < segs; seg++ )
        {
          ...
          /// SOLUTION
          if ( !opt_tryout1 && show_theta )
            v2 = pVect( slice_r0 * cosf(theta0), y0, slice_r0 * sinf(theta0) );
      ...
      /// SOLUTION
      if ( !opt_tryout1 && show_eta ) { glEnd(); glBegin( GL_LINE_STRIP ); }
  ...
  /// SOLUTION -- Draw Axes
  //
  if ( !opt_tryout1 && opt_show )
    {
      const pColor color_arrow(0xeeeeee);

      const float ab = 0.01; // Axis block radius.

      auto rot90_a = [](int a, pVect v)
        {
          switch ( a ) {
          case 0:  return pVect{ v.x, -v.z, v.y };
          case 1:  return pVect{ v.z, v.y, -v.x };
          default: return pVect{ -v.y, v.x, v.z };
          }
        };

      auto emit_face = [&](pCoor c, pVect sh, pVect lo)
        {
          pNorm n = cross(sh,lo);
      ...
      auto emit_block = [&](int axis)
        {
          pCoor co(ab, ab, ab);  co[axis] = 0;
          pVect lo(0,0,0);       lo[axis] = 1.5;
          pVect sh(0,0,0);       sh[(axis+1)%3] = -2*ab;
          float color[3] = {0};  color[axis] = 0.9;
          glColor3fv(color);
          for ( int i=0; i<4; i++ )
            {
              emit_face(co,sh,lo);
