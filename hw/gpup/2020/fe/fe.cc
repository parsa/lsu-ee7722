...
//
 /// Pre-Final Code.
 //
 //  Based on Homework 3, which itself was based on proj-base/links.
 //
...
public:
  ...
  float opt_tryoutf;
  uint opt_sphere_shader;
  vector< pair<string,pShader*> > sp_shaders;
  map<size_t,bool> shader_is_true;
...
World::init_graphics()
{
  ...
  texid_hw = pBuild_Texture_File("hw-assign.png", false,255);
...
World::cb_keyboard()
{
  ...
  case 'z':
    if ( ++opt_sphere_shader >= sp_shaders.size() ) opt_sphere_shader = 0;
    break;
  case ' ':
...
World::init(int argc, char **argv)
{
  ...
  string sol_name("fe-sol");
  const char* const mt_path =
    this_exe_name.substr(0,sol_name.size()) == sol_name ?
    "fe-shdr-sol.cc" : "fe-shdr.cc";
