...
public:
  ...
  void start()
  {
    ...
    const int thd_per_mp_lg_min = 6;
    const int thd_per_mp_lg_max = 12;
    const int block_lg_min = 5;

    for ( int thd_per_mp_lg = thd_per_mp_lg_min;
          thd_per_mp_lg < thd_per_mp_lg_max;  thd_per_mp_lg++ )
      {
        const int thd_per_mp = 1 << thd_per_mp_lg;
        printf("---- Configurations with %d threads per multiprocessor:\n",
               thd_per_mp);

        for ( int thd_per_block_lg = block_lg_min;
              thd_per_block_lg <= thd_per_mp_lg; thd_per_block_lg++ )
          {
            const int thd_per_block = 1 << thd_per_block_lg;
            if ( thd_per_block > max_block_size ) break;
            const int bl_per_mp = 1 << ( thd_per_mp_lg - thd_per_block_lg );
            run(thd_per_block,bl_per_mp,false);
          }
      }

    /// SOLUTION - PROBLEM 3
    //
    run(256,0,false);  // Run some loopless configurations.
    run(512,0,false);  // Run some loopless configurations.
    run(1024,0,false);  // Run some loopless configurations.

  }

  void run(int block_size, int bl_per_mp = 1, bool short_stride = false)
  {
    ...
    printf("Grid Dim %6d,  Block Dim %4d,  BL/MP %7.1f,  "
           "TH/MP %7.0f  Version %c\n",
           dg.x, db.x,
    ...
    for ( int cpu_round=0; cpu_round<cpu_rounds; cpu_round++ )
      {
        ...
        for ( int i=0; i<gpu_rounds; i++ )
          dots_launch(dg,db,version,array_size);
