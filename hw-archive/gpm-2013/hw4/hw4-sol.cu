...
//
 ...
 /// Homework 4 Solution
 //
 //  Search for SOLUTION to find start of solution.
 //
 // Solution writeup: http://www.ece.lsu.edu/koppel/gp/2013/hw04_sol.pdf
 //
 /// Your Name: David Koppelman
...
reduce_method_4(float thd_sum)
{
  ...
  // Compute a warp number for this thread.
  //
  const int warp_num = threadIdx.x >> warp_lg;

  // Compute this thread's position within its warp, called a lane.
  // The lane number will be from 0 to 31 (with a warp size of 32).
  //
  const int lane = threadIdx.x & ( warp_size - 1 );

  // Compute a per-warp sum.
  //
  for ( int dist = half_warp_size;  dist;  dist >>= 1 )
    if ( lane < dist )
      {
  ...
  // At this point there is a sum for each warp. The sum for warp
  // number i is in shared_sum[ i * warp_size ]. Before adding these
  // values up, move them to consecutive positions in a new shared
  // array wshared_sum.
  //
  // They are being moved to consecutive positions to avoid bank
  // conflicts. That is, if they were not moved, thread i would
  // be accessing element shared_sum[ i * warp_size ]. Threads 0-31
  // would all be accessing the same bank.  
  //
  // They are being moved to a second array, wshared_sum, to avoid
  ...
  // Write zero in unused wshared_sum elements.
  //
  const int pull_from = threadIdx.x << warp_lg;
  if ( threadIdx.x < warp_size && pull_from >= blockDim.x )
    wshared_sum[ lane ] = 0;

  // At this point we only need threads 0-15.
  //
  if ( threadIdx.x >= half_warp_size ) return;

  Elt_Type our_osum = wshared_sum[ threadIdx.x ];

  // Add up sums computed above.
  //
  for ( int dist = half_warp_size;  dist;  dist >>= 1 )
    if ( lane < dist )
      {
