...
World::init()
{
  ...
  const char* const file = "spring-shdr.cc";
  ...
  opt_gravity = true;
  opt_interpen_method = 1;
  opt_test = false;

  interp_seg_per_block = 8;
  variable_control.insert_power_of_2(interp_seg_per_block,"Seg / Block");
...
# define BLINK(txt,pad) ( blink_visible ? txt : pad )
  ...
  ogl_helper.fbprintf
    ("Physics: %s ('a')  Pause: %s ('p')  Gravity: %s ('g')  Grabbed: %s ('%s')  Interpenetration: %s ('i')  Debug: %d ('t')\n",
     ...
     opt_end_fixed ? "B" : "b",
     opt_interpen_method ?
     ( opt_interpen_method == 1 ? "Method 1" : "Method 2" )
     : BLINK("OFF","   "),
     opt_test);
              ...
              if ( opt_interpen_method )
                time_step_intersect_launch
                  (phys_helix_segments/interp_seg_per_block,256,
                   (float4*)helix_position.get_dev_addr(),
                   (float3*)helix_velocity.get_dev_addr()
                   );
...
World::cb_keyboard()
{
  ...
  case 'i': case 'I': opt_interpen_method++;
    if ( opt_interpen_method > 1 ) opt_interpen_method = 0;
    cuda_constants_stale = true;
    break;
